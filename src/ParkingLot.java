...
*/
    ...
    /*
    转换为0基数是为了正确访问Java中的列表索引。如果不进行此转换，会导致索引越界或访问错误的元素。具体来说：

        票据ID中的楼层号和停车位号是从1开始的（例如，楼层1表示第一层，停车位1表示第一个停车位）。
        然而，Java中的列表索引是从0开始的（例如，索引0表示第一层或第一个停车位）。
        因此，需要将票据ID中的楼层号和停车位号减1，以便在代码中正确访问对应的列表元素。例如：
        
        楼层号为2，停车位号为5，在票据ID中表示为 PL123_2_5。
        在代码中，需要访问 slots.get(1).get(4) 对应的停车位（索引从0开始）。
    */
